...
static const u16 sBerryMasterWifePhrases[][2] = {
    ...
    [PHRASE_OVERWHELMING_LATIAS - 1] = {EC_WORD_OVERWHELMING, EC_POKEMON(KURANPU)},
    [PHRASE_COOL_LATIOS - 1]         = {EC_WORD_COOL, EC_POKEMON(UNICROWN)},
...
static void TryAddInterviewObjectEvents(void)
{
    ...
    spriteId = CreateObjectGraphicsSprite(
        gSaveBlock2Ptr->playerGender != FEMALE ? OBJ_EVENT_GFX_RIVAL_BRENDAN_NORMAL : OBJ_EVENT_GFX_RIVAL_MAY_NORMAL,
        SpriteCallbackDummy,
        52,
        40,
        0);
...
u16 EasyChat_GetNumWordsInGroup(u8 groupId)
{
    if (groupId == EC_GROUP_POKEMON)
        return GetPokedexCount(FLAG_GET_SEEN);
...
static void SetUnlockedEasyChatGroups(void)
{
    ...
    if (GetPokedexCount(FLAG_GET_SEEN))
...
static u16 SetSelectedWordGroup_GroupMode(u16 groupId)
{
    ...
    if (groupId == EC_GROUP_POKEMON || groupId == EC_GROUP_MOVE_1  || groupId == EC_GROUP_MOVE_2)
    {
        list = gEasyChatGroups[groupId].wordData.valueList;
