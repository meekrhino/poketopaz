...
COMMON_DATA u8 gDefeatedInBattle[PARTY_SIZE];
...
const u8 gTypeEffectiveness[366] =
{
    ...
    TYPE_FIGHTING, TYPE_LIGHT, TYPE_MUL_NOT_EFFECTIVE,
    ...
    TYPE_POISON, TYPE_LIGHT, TYPE_MUL_NOT_EFFECTIVE,
    ...
    TYPE_GHOST, TYPE_LIGHT, TYPE_MUL_SUPER_EFFECTIVE,
    ...
    TYPE_LIGHT, TYPE_GRASS, TYPE_MUL_NOT_EFFECTIVE,
    TYPE_LIGHT, TYPE_PSYCHIC, TYPE_MUL_NO_EFFECT,
...
{
    {TRAINER_CLASS_ORION_DEFENDER, 5},
...
void SwitchInClearSetData(void)
{
    ...
    if (gBattleMoves[gCurrentMove].effect == EFFECT_BATON_PASS)
    {
        ...
        gStatuses3[gActiveBattler] &= (STATUS3_LEECHSEED_BATTLER | STATUS3_LEECHSEED | STATUS3_ALWAYS_HITS | STATUS3_PERISH_SONG | STATUS3_ROOTED | STATUS3_MUDSPORT | STATUS3_WATERSPORT | STATUS3_GRAVITY_BOUND);
...
void FaintClearSetData(void)
{
    ...
    gProtectStructs[gActiveBattler].usedSpellboundMove = FALSE;
...
static void TryDoEventsBeforeFirstTurn(void)
{
    ...
    if (AbilityBattleEffects(ABILITYEFFECT_INTIMIDATE1, 0, 0, 0, 0) != 0)
        return;
    if (AbilityBattleEffects(ABILITYEFFECT_CONFOUND1, 0, 0, 0, 0) != 0)
        return;
    ...
    {
        if (ItemBattleEffects(ITEMEFFECT_ON_SWITCH_IN, gBattlerByTurnOrder[gBattleStruct->switchInItemsCounter], FALSE, 0))
            effect++;
