...
static const u32 sStatusFlagsForMoveEffects[NUM_MOVE_EFFECTS] =
{
    ...
    [MOVE_EFFECT_LEECH_SEED]     = STATUS3_LEECHSEED,
};
...
static void Cmd_critcalc(void)
{
    ...
    if ((gBattleMons[gBattlerTarget].ability != ABILITY_BATTLE_ARMOR && gBattleMons[gBattlerTarget].ability != ABILITY_SHELL_ARMOR)
     ...
     && !(gBattleTypeFlags & (BATTLE_TYPE_BRADLEY_TUTORIAL | BATTLE_TYPE_FIRST_BATTLE))
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if (IS_MOVE_SPECIAL(gBattleMoves[gCurrentMove]) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE))
                {
...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    else if (gBattleCommunication[MOVE_EFFECT_BYTE] == MOVE_EFFECT_LEECH_SEED)
    {
        if (gStatuses3[gBattlerTarget] & sStatusFlagsForMoveEffects[gBattleCommunication[MOVE_EFFECT_BYTE]]
...
static void ChooseMonToSendOut(u8 slotId)
{
    ...
    *(gBattleStruct->monToSwitchIntoId + gActiveBattler) = PARTY_SIZE;
    ...
    BtlController_EmitChoosePokemon(BUFFER_A, partyAction, slotId, ABILITY_NONE, gBattleStruct->battlerPartyOrders[gActiveBattler]);
    MarkBattlerForControllerExec(gActiveBattler);
}
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_CLEAR_WEATHER:
        gBattleWeather = B_WEATHER_STILL;
    ...
    case VARIOUS_SET_DARKNESS:
        if (gBattleWeather & B_WEATHER_DARKNESS || gBattleWeather & B_WEATHER_STILL)
        {
            gMoveResultFlags |= MOVE_RESULT_MISSED;
        ...
        {
            gBattleWeather = B_WEATHER_DARKNESS_TEMPORARY;
    ...
    case VARIOUS_SPECTRAL_RAY:
        if (gBattleWeather & B_WEATHER_DARKNESS || gBattleWeather & B_WEATHER_RAIN)
    ...
    case VARIOUS_SOLAR_CELL:
        if ((!(gBattleWeather & B_WEATHER_ANY) || (gBattleWeather & B_WEATHER_SUN))
...
static void Cmd_setrain(void)
{
    if (gBattleWeather & B_WEATHER_RAIN || gBattleWeather & B_WEATHER_STILL)
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_WEATHER_FAILED;
    }
...
static void Cmd_statbuffchange(void)
{
    ...
    if (ChangeStatBuffs(statValue, statId, gBattlescriptCurrInstr[1], jumpPtr) == STAT_CHANGE_WORKED)
    {
        u8 copycatRecord = 0;
...
static void Cmd_setlightscreen(void)
{
    ...
    if (gCurrentMove == MOVE_WATER_WALL)
    {
        ...
        else
        {
            ...
            if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE && CountAliveMonsInBattle(BATTLE_ALIVE_ATK_SIDE) == 2)
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SET_WATERWALL_DOUBLE;
            else
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SET_WATERWALL_SINGLE;
        }
    }
    else
    {
...
static void Cmd_setsandstorm(void)
{
    if (gBattleWeather & B_WEATHER_SANDSTORM || gBattleWeather & B_WEATHER_STILL)
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_WEATHER_FAILED;
    }
...
static void Cmd_setsunny(void)
{
    if (gBattleWeather & B_WEATHER_SUN || gBattleWeather & B_WEATHER_STILL)
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_WEATHER_FAILED;
    }
...
static void Cmd_sethail(void)
{
    if (gBattleWeather & B_WEATHER_HAIL || gBattleWeather & B_WEATHER_STILL)
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_WEATHER_FAILED;
    }
...
static void Cmd_removelightscreenreflect(void) // brick break
{
    ...
    if (gSideTimers[opposingSide].reflectTimer || gSideTimers[opposingSide].lightscreenTimer || gSideTimers[opposingSide].waterwallTimer)
    {
        gSideStatuses[opposingSide] &= ~SIDE_STATUS_REFLECT;
        gSideStatuses[opposingSide] &= ~SIDE_STATUS_LIGHTSCREEN;
        gSideStatuses[opposingSide] &= ~SIDE_STATUS_WATERWALL;
        gSideTimers[opposingSide].reflectTimer = 0;
        gSideTimers[opposingSide].lightscreenTimer = 0;
        gSideTimers[opposingSide].waterwallTimer = 0;
        gBattleScripting.animTurn = 1;
