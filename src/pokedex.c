    ...
    case 8:
        if (!gPaletteFade.active)
        {
            ...
            if (gTasks[taskId].data[3] == 0)
            {
                ...
                PlayCry2(sPokedexListItem->dexNum, 0, 0x7D, 0xA);
            }
            else
            {
                gMain.state++;
...
static void Task_LoadCryScreen(u8 taskId)
{
    ...
    case 4:
        ...
        break;
    case 5:
        gTasks[taskId].tMonSpriteId = CreateMonSpriteFromDexNumber(sPokedexListItem->dexNum, 48, 56, 0);
        gSprites[gTasks[taskId].tMonSpriteId].oam.priority = 0;
...
static void Task_DisplayCaughtMonDexPage(u8 taskId)
{
    ...
    case 3:
        ...
        break;
    case 4:
        spriteId = CreateMonSpriteFromDexNumber(dexNum, 48, 56, 0);
        gSprites[spriteId].oam.priority = 0;
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0x10, 0, RGB_BLACK);
        SetVBlankCallback(gPokedexVBlankCB);
        gTasks[taskId].tMonSpriteId = spriteId;
...
{
    u8 image[32 * 4];
    const u8 * r12 = gMonFootprintTable[dexNum];
    u16 r5 = 0;
    u16 i;
    u16 j;
