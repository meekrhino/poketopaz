...
static const union AnimCmd *const sSpriteAnimTable_HoennText[] =
{
...
static const struct SpriteTemplate sHoennTextSpriteTemplate =
{
    .tileTag = TAG_DEX_INTERFACE,
    .paletteTag = TAG_DEX_INTERFACE,
    .oam = &sOamData_InterfaceText,
    .anims = sSpriteAnimTable_HoennText,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCB_SeenOwnInfo,
};
...
{
    [DEX_MODE_CALDERA_KIRANT]    = {gText_DexHoennDescription, gText_DexHoennTitle},
    {},
};
...
static const u8 sPokedexModes[] = {DEX_MODE_CALDERA_KIRANT};
...
void ResetPokedex(void)
{
    ...
    gUnusedPokedexU8 = 0;
    gSaveBlock2Ptr->pokedex.mode = DEX_MODE_CALDERA_KIRANT;
...
static void ResetPokedexView(struct PokedexView *pokedexView)
{
    ...
    pokedexView->pokedexList[NUM_SPECIES].seen = FALSE;
    pokedexView->pokedexList[NUM_SPECIES].owned = FALSE;
    pokedexView->pokemonListCount = 0;
    pokedexView->selectedPokemon = 0;
    pokedexView->selectedPokemonBackup = 0;
    pokedexView->dexMode = DEX_MODE_CALDERA_KIRANT;
    pokedexView->dexModeBackup = DEX_MODE_CALDERA_KIRANT;
...
void CB2_OpenPokedex(void)
{
    ...
    case 2:
        ...
        sPokedexView->dexMode = DEX_MODE_CALDERA_KIRANT;
        ...
        sPokedexView->seenCount = GetPokedexCount(FLAG_GET_SEEN);
        sPokedexView->ownCount = GetPokedexCount(FLAG_GET_CAUGHT);
...
static void CreatePokedexList(u8 dexMode, u8 order)
{
    ...
    case ORDER_NUMERICAL:
        for (i = 0; i < CALDERA_KIRANT_DEX_COUNT; i++)
        {
            dexNum = i + 1;
    ...
    case ORDER_ALPHABETICAL:
        for (i = 0; i < NUM_SPECIES - 1; i++)
        {
            ...
            if (dexNum <= dexCount && GetSetPokedexFlag(dexNum, FLAG_GET_SEEN))
            {
    ...
    case ORDER_HEAVIEST:
        for (i = NUM_SPECIES - 1; i >= 0; i--)
        {
            ...
            if (dexNum <= dexCount && GetSetPokedexFlag(dexNum, FLAG_GET_CAUGHT))
            {
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].dexNum = dexNum;
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].seen = TRUE;
...
static u32 CreatePokedexMonSprite(u16 num, s16 x, s16 y)
{
    ...
    for (i = 0; i < MAX_MONS_ON_SCREEN; i++)
    {
        if (sPokedexView->monSpriteIds[i] == 0xFFFF)
        {
            u8 spriteId = CreateMonSpriteFromDexNumber(num, x, y, i);

            gSprites[spriteId].oam.affineMode = ST_OAM_AFFINE_NORMAL;
            gSprites[spriteId].oam.priority = 3;
            gSprites[spriteId].data[0] = 0;
            gSprites[spriteId].data[1] = i;
            gSprites[spriteId].data[2] = num;
...
static void Task_LoadInfoScreen(u8 taskId)
{
    ...
    case 5:
        if (!gTasks[taskId].tMonSpriteDone)
        {
            gTasks[taskId].tMonSpriteId = (u16)CreateMonSpriteFromDexNumber(sPokedexListItem->dexNum, 48, 56, 0);
    ...
    case 8:
        if (!gPaletteFade.active)
        {
            ...
            if (!gTasks[taskId].tSkipCry)
            {
                ...
                PlayCry_NormalNoDucking(sPokedexListItem->dexNum, 0, CRY_VOLUME_RS, CRY_PRIORITY_NORMAL);
            }
            else
            {
                gMain.state++;
...
static void Task_LoadCryScreen(u8 taskId)
{
    ...
    case 4:
        ...
        break;
    case 5:
        gTasks[taskId].tMonSpriteId = CreateMonSpriteFromDexNumber(sPokedexListItem->dexNum, MON_PAGE_X, MON_PAGE_Y, 0);
        gSprites[gTasks[taskId].tMonSpriteId].oam.priority = 0;
...
static void Task_HandleCryScreenInput(u8 taskId)
{
    ...
    {
        LoadPlayArrowPalette(TRUE);
        CryScreenPlayButton(sPokedexListItem->dexNum);
        return;
    }
...
static void Task_LoadSizeScreen(u8 taskId)
{
    ...
    case 5:
        ...
        break;
    case 6:
        spriteId = CreateMonSpriteFromDexNumber(sPokedexListItem->dexNum, 88, 56, 1);
        gSprites[spriteId].oam.affineMode = ST_OAM_AFFINE_NORMAL;
...
static void Task_DisplayCaughtMonDexPage(u8 taskId)
{
    ...
    case 3:
        ...
        break;
    case 4:
        spriteId = CreateMonSpriteFromDexNumber(dexNum, MON_PAGE_X, MON_PAGE_Y, 0);
        gSprites[spriteId].oam.priority = 0;
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0x10, 0, RGB_BLACK);
        SetVBlankCallback(gPokedexVBlankCB);
        gTasks[taskId].tMonSpriteId = spriteId;
...
{
    u8 footprint[32 * 4];
    const u8 * footprintGfx = gMonFootprintTable[dexNum];
    u16 tileIdx = 0;
    u16 i, j;
...
u16 CreateMonSpriteFromDexNumber(u16 species, s16 x, s16 y, u16 paletteSlot)
{
        return CreateMonPicSprite_HandleDeoxys(species, SHINY_ODDS, GetPokedexMonPersonality(species), TRUE, x, y, paletteSlot, TAG_NONE);
}
...
static int DoPokedexSearch(u8 dexMode, u8 order, u8 abcGroup, u8 bodyColor, u8 type1, u8 type2)
{
    ...
    if (abcGroup != 0xFF)
    {
        for (i = 0, resultsCount = 0; i < sPokedexView->pokemonListCount; i++)
        {
            ...
            species = sPokedexView->pokedexList[i].dexNum;
...
#define tUnused                 data[2]
#define tUnused2                data[3]
...
{
    u8 dexMode = DEX_MODE_CALDERA_KIRANT;
...
static void PrintSelectedSearchParameters(u8 taskId)
{
    ...
    PrintSearchText(gText_DexHoennTitle, 0x2D, 0x51);
}
...
static void SetDefaultSearchModeAndOrder(u8 taskId)
{
    ...
    {
    default:
    case DEX_MODE_CALDERA_KIRANT:
        selected = DEX_MODE_CALDERA_KIRANT;
        break;
    }
