...
static void HandleInputChooseMove(void)
{
    ...
    if (JOY_NEW(A_BUTTON))
    {
        ...
        {
            if (!(moveTarget & (MOVE_TARGET_RANDOM | MOVE_TARGET_BOTH | MOVE_TARGET_DEPENDS | MOVE_TARGET_FOES_AND_ALLY | MOVE_TARGET_ALL | MOVE_TARGET_OPPONENTS_FIELD | MOVE_TARGET_USER)))
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK)
    {
        ...
        else
        {
            switch(gLinkPlayers[GetMultiplayerId()].gender) {
                ...
                case NONBINARY:
                    trainerPicId = TRAINER_BACK_PIC_PLAYER_N;
            }
        }
    }
    else
    {
        trainerPicId = gPlayerBackPicGenderTable[gSaveBlock2Ptr->playerGender];
    }
...
static void PlayerHandleIntroTrainerBallThrow(void)
{
    ...
    LoadCompressedPalette(gTrainerBackPicPaletteTable[gPlayerBackPicGenderTable[gSaveBlock2Ptr->playerGender]].data, OBJ_PLTT_ID(paletteNum), PLTT_SIZE_4BPP);
